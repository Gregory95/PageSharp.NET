...
`PageSharp` provides a small set of helpers to standardize paging (slicing) of `IQueryable<T>` sources and to return a page of results together with paging metadata.
...
- `PageSharp<T>` — a generic page container that holds the items for a single page and metadata such as current page, total pages, page size and total count.
...
`dotnet add package PageSharp --version 1.0.3`

- Or add a `PackageReference` to your project file:

`<PackageReference Include="PageSharp" Version="1.0.3" />`
...
The `PageSharp.Create` / `CreateAsync` methods expect a zero-based page index (0 = first page). If you are using `PagingParameters.PageNumber` (which is1-based), pass `pagingParams.PageNumber -1` to the `Create` / `CreateAsync` methods.
...
var page = await PageSharp<Product>.CreateAsync(query, pagingParams.PageNumber -1, pagingParams.PageSize, CancellationToken.None);
...
var syncPage = PageSharp<Product>.Create(query, pagingParams.PageNumber -1, pagingParams.PageSize);
```
